...
//#endif
	...
	while (nvp && (b = strsep(&nvp, "<")) != NULL) {
		...
		port = *tlsport ? safe_atoi(tlsport) : 0;
...
start_ddns(char *caller, int isAidisk)
{
	...
	else if (strcmp(server, "DEDYN.IO") == 0)
		service = "default@dedyn.io";
	else if (strcmp(server, "WWW.TUNNELBROKER.NET") == 0) {
...
void clean_afc_power(void)
{
	...
	snprintf(buff, sizeof(buff), "wl -i %s afc_info -1", nvram_safe_get(wlif));
	system(buff);
...
unsigned long off_pd() {
	...
	if(fp) {
		...
		if(fgets(buf, sizeof(buf), fp) != NULL) {
			...
			snprintf(cmd, sizeof(cmd), "i2cset -y 0 0x21 0x20 0x%lx", val);
			system(cmd);
		}
		pclose(fp);
	}
...
#ifdef	RTCONFIG_MULTILAN_CFG
			if(sessionTime){
			  	nvram_set_int("cp_sessiontime", safe_atoi(sessionTime));
			}
#else
			  nvram_set("cp_sessiontime", sessionTime);
#endif
			  nvram_set("cp_awaytime", awayTime);
			  nvram_set_int("cp_authtime", safe_atoi(nvram_get("captive_portal_adv_idle_timeout")));
			  nvram_set_int("cp_authtype", safe_atoi(authType));
	...
	nv = nvp = strdup(nvram_safe_get("captive_portal"));
		...
		if(strstr(cr_en,"<") || strstr(cr_en,">")){
			...
			snprintf(brdName,strlen(brdName_en)+1,brdName_en);
...
#endif
#if 0
			  if (BYPASS_CP == safe_atoi(typeIdx))
...
#ifdef	RTCONFIG_MULTILAN_CFG
			  nvram_set_int("chilli_sessiontime", safe_atoi(sessionTime));
#else
			  nvram_set_int("chilli_sessiontime", 60 * safe_atoi(sessionTime));
#endif
			  if (EXTERNAL_UI == safe_atoi(UIType)){
			      nvram_set("external_UI", UIPath);
			  }else
			      nvram_unset("external_UI");

			  if (INTERNAL_UI == safe_atoi(UIType)){
			  ...
			  if((NULL == enPass) || (0 == safe_atoi(enPass))) nvram_set("captive_portal_passcode", "");
...
//#if defined(RTCONFIG_JFFS2LOG) && defined(RTCONFIG_JFFS2)
	...
	else if(strcmp(script, "resetdefault") == 0)
	{
		...
		else			ate_upgrade_reboot = safe_atoi(cmd[1]);
		if(cmd[2] == NULL)	ate_upgrade_reset = 0;
		else			ate_upgrade_reset = safe_atoi(cmd[2]);
		if(cmd[3] == NULL)	ate_upgrade_bootnew = 0;
		else			ate_upgrade_bootnew = safe_atoi(cmd[3]);
#if RTCONFIG_PIPEFW
...
/* TODO fix fini_wl() for BCM USBAP */
		...
		if(action & RC_SERVICE_START) {
			...
			char ai_upgrade_file[128] = "firmware/aisom.swu", webs_state_ai_info[64] = {0}, abs_ai_upgrade_file[128] = {0};
			snprintf(webs_state_ai_info, sizeof(webs_state_ai_info), "%s", nvram_safe_get("webs_state_ai_info"));
			snprintf(abs_ai_upgrade_file, sizeof(abs_ai_upgrade_file), "/ai/%s", ai_upgrade_file);
		...
		#endif
#ifdef RTCONFIG_AI_SERVICE
			/* aisom.swu is for normal upgrade and always has higher priority */
			if (!f_exists(abs_ai_upgrade_file) && strlen(webs_state_ai_info) > 8) {
				memset(ai_upgrade_file, 0, sizeof(ai_upgrade_file));
				memset(abs_ai_upgrade_file, 0, sizeof(abs_ai_upgrade_file));

				snprintf(ai_upgrade_file, sizeof(ai_upgrade_file),
					"firmware/%s_%s.swu",
					nvram_safe_get("ai_productid"),
					webs_state_ai_info);
				snprintf(abs_ai_upgrade_file, sizeof(abs_ai_upgrade_file), "/ai/%s", ai_upgrade_file);
				
				_dprintf("ai upgrade file : %s \n", abs_ai_upgrade_file);
			}
			/* aisom.swu or other firmware exist and webs_state_ai_flag equals to 1 or 2(upgrade or force upgrade)*/
			if(f_exists(abs_ai_upgrade_file))
			{
				_dprintf("Start to upgrade ai board [%s]\n", ai_upgrade_file);
				logmessage("[AISRV]", "Start to upgrade ai board [%s]", ai_upgrade_file);
				// reset ai board upgrade state and error
				nvram_set("webs_state_ai_upgrade", "0");
				nvram_set("webs_state_ai_error", "0");
				start_ai_upgrade(ai_upgrade_file);
			}
			else
			{
				_dprintf("No need to upgrade ai board [%s]\n", ai_upgrade_file);	
				logmessage("[AISRV]", "No need to upgrade ai board [%s]", ai_upgrade_file);	
			}
			if(f_exists(abs_ai_upgrade_file)){
				ai_done = remove(abs_ai_upgrade_file);
				if(ai_done)
				{
					_dprintf("Fail to remove [%s]\n", ai_upgrade_file);	
					logmessage("[AISRV]", "Fail to remove [%s]", ai_upgrade_file);
				}
				else
				{
					_dprintf("remove [%s]\n", ai_upgrade_file);	
					logmessage("[AISRV]", "remove [%s]", ai_upgrade_file);
				}
			}
#endif
...
/* TODO fix fini_wl() for BCM USBAP */
		...
		modprobe_r("udf");
#endif
#if (defined(RTCONFIG_TFAT) || defined(RTCONFIG_TUXERA_NTFS) || defined(RTCONFIG_TUXERA_HFS)) && (defined(WIFI7_SDK_20250506) || defined(WIFI8_SDK_20251126) || defined(GSBE18000) || defined(GSBE12000) || defined(GS7_PRO) || defined(GT7) || defined(GS7_PRO_MAX))
...
_dprintf("multipath(%s): unit_now: (%d, %d, %s), unit_next: (%d, %d, %s).\n", mode, unit_now, state_now, if_now, unit_next, state_next, if_next);
		...
		if (strchr(string, ','))
		{
			...
			cpu = strsep(&ddr, ",");
			cpu_clock=safe_atoi(cpu);
			ddr_clock=safe_atoi(ddr);
		}
		else
			cpu_clock=safe_atoi(string);
...
check_ddr_done:
		...
		if(cmd[1]) {
			...
			if(action & RC_SERVICE_START)
			{
				...
				check_wan_if(safe_atoi(cmd[1]));

#endif
#ifdef RTCONFIG_MULTISERVICE_WAN
#if defined(HND_ROUTER)
				config_mswan(safe_atoi(cmd[1]));
#endif
#endif
				start_wan_if(safe_atoi(cmd[1]));
		...
		if((action&RC_SERVICE_STOP) && (action & RC_SERVICE_START)) {
			int unit = cmd[1] ? safe_atoi(cmd[1]) : 0;
...
//		if(action & RC_SERVICE_STOP) stop_moca_mps();
	...
	else if (strcmp(script, "ai_request_action") == 0)
	{
		_dprintf("AI REQ options count : %d\n", count);
		if(count > 1)
		{
			int cmd_idx = 1;
			int req_flag = 0;
			for(cmd_idx = 1; cmd_idx<count; cmd_idx++)
			{
				if(!strcmp(cmd[cmd_idx], "query")) {req_flag |= AI_STA_REQ;}
				else if(!strcmp(cmd[cmd_idx], "update")) {req_flag |= AI_UPD_REQ;}
				else if(!strcmp(cmd[cmd_idx], "apply")) {req_flag |= AI_SET_REQ;}
				else if(!strcmp(cmd[cmd_idx], "reset")) {req_flag |= AI_RST_REQ;}
				else if(!strcmp(cmd[cmd_idx], "feedback")) {req_flag |= AI_FBK_REQ;}
				else if(!strcmp(cmd[cmd_idx], "rescue")) {req_flag |= AI_RSC_REQ;}
				else {_dprintf("%s INVALID AI REQ options!\n", cmd[cmd_idx]);}
			}
			if(action & RC_SERVICE_START) send_ai_request(req_flag);
		}
		else _dprintf("MISS AI REQ options!\n");
	}
#endif /* RTCONFIG_AI_SERVICE */
	...
	else if (strcmp(script, "afcd_harness") == 0)
	{
		 ...
		 * */
		stop_afcd();
		afcd_harness_config(safe_atoi(cmd[1]));
		start_afcd();
	}
	...
	{
		int rssi = safe_atoi(cmd[1]);
		int tx = safe_atoi(cmd[2]);
		int channel = safe_atoi(cmd[3]);
		int band = safe_atoi(cmd[4]);
		int is_wired = safe_atoi(cmd[5]);
...
//#ifdef RTCONFIG_CLOUDSYNC
	else if (strcmp(script, "cloudsync") == 0)
	{
		...
		if(action&RC_SERVICE_STOP){
			if(cmd[1])
				stop_cloudsync(safe_atoi(cmd[1]));
			else
				stop_cloudsync(-1);
		}
	...
	else if (strcmp(script, "sdn") == 0)
	{
		if(count > 1)
		{
			for(i=1;i<count;i++)
			{
				if(cmd[i])
				{
					handle_sdn_feature(safe_atoi(cmd[i]), SDN_FEATURE_ALL, action);
				}
			}
		}
		else
	...
	else if(strcmp(script, "urlfilter_sdn") == 0)
	{
		if(count > 1)
		{
			for(i=1;i<count;i++)
			{
				if(cmd[i])
				{
					handle_sdn_feature(safe_atoi(cmd[i]), SDN_FEATURE_URL_FILTER, 0);
				}
			}
		}
		else
	...
	else if(strcmp(script, "urlfilter_idx") == 0)
	{
		if(count > 1)
		{
			for(i=1;i<count;i++)
			{
				if(cmd[i])
				{
					update_URLFilter(safe_atoi(cmd[i]));
				}
			}
		}
		else
	...
	else if(strcmp(script, "nwsfilter_sdn") == 0)
	{
		if(count > 1)
		{
			for(i=1;i<count;i++)
			{
				if(cmd[i])
				{
					handle_sdn_feature(safe_atoi(cmd[i]), SDN_FEATURE_NWS_FILTER, 0);
				}
			}
		}
		else
	...
	else if(strcmp(script, "nwsfilter_idx") == 0)
	{
		if(count > 1)
		{
			for(i=1;i<count;i++)
			{
				if(cmd[i])
				{
					update_NwServiceFilter(safe_atoi(cmd[i]));
				}
			}
		}
		else
	...
	else if(strcmp(script, "sdn_wan") == 0)
	{
		if(count > 1)
		{
			...
			{
				if(cmd[i])
					handle_sdn_feature(safe_atoi(cmd[i]), SDN_FEATURE_WAN, 0);
			}
		}
		else
		{
	...
	else if (strcmp(script, "dnsmasq") == 0)
	{
		...
		if(count > 1)
		{
			for(i=1;i<count;i++)
			{
				if(cmd[i])
				{
					if(action & RC_SERVICE_STOP) stop_dnsmasq(safe_atoi(cmd[i]));
					if(action & RC_SERVICE_START) start_dnsmasq(safe_atoi(cmd[i]));
				}
			}
		}
		else
	...
	else if (strcmp(script, "stubby") == 0)
	{
		...
		if(count > 1)
		{
			for(i=1;i<count;i++)
			{
				if(cmd[i])
				{
					if(action & RC_SERVICE_STOP) stop_stubby(safe_atoi(cmd[i]));
					if(action & RC_SERVICE_START) start_stubby(safe_atoi(cmd[i]));
				}
			}
		}
		else
	...
	{
		int wan_unit = (count > 1) ? safe_atoi(cmd[1]) : wan_primary_ifunit();
		int lan_unit = (count > 2) ? safe_atoi(cmd[2]) : 0;
...
retry_wps_enr:
 ...
 && !defined(RTCONFIG_CONCURRENTREPEATER)
	...
	{
		if(cmd[1]&& (safe_atoi(cmd[1]) != nvram_get_int("wlc_mode"))) {
			nvram_set_int("wlc_mode", safe_atoi(cmd[1]));
			if(nvram_match("lan_proto", "dhcp") && safe_atoi(cmd[1])==0) {
				nvram_set("lan_ipaddr", nvram_default_get("lan_ipaddr"));
			}
#if defined(RTCONFIG_QCA_LBD)
	...
	else if (strcmp(script, "snmpd") == 0)
	{
		...
		if (strlen(script) == 10) {
			if (action & RC_SERVICE_STOP) stop_ovpn_client(safe_atoi(&script[9]));
			if (action & RC_SERVICE_START) start_ovpn_client(safe_atoi(&script[9]));
		}
	}
	else if (strncmp(script, "vpnserver" ,9) == 0) {
		if (strlen(script) == 10) {
			if (action & RC_SERVICE_STOP) stop_ovpn_server(safe_atoi(&script[9]));
			if (action & RC_SERVICE_START) start_ovpn_server(safe_atoi(&script[9]));
		}
	}
		...
		if (action & RC_SERVICE_START){
			...
			if(nvram_match("vpnc_proto", "openvpn")){
				...
				while (nv && (b = strsep(&nvp, "<")) != NULL) {
					...
					if (server && (unit = safe_atoi(server)) != openvpnc_unit)
						continue;
...
_dprintf("test 1. usbreset_active=%s, usbreset_num=%s.\n", usbreset_active, usbreset_num);
		...
		if(strlen(usbreset_num) > 0 && strlen(usbreset_active) > 0 && strcmp(usbreset_active, "0")){
			reset_num = safe_atoi(usbreset_num);
...
_dprintf("test 2. turn off the USB power during %d seconds.\n", reset_seconds[reset_num]);
	...
	else if (strcmp(script, "auto46det") == 0)
	{
		...
		if (count == 3 && cmd[1] != NULL && cmd[2] != NULL)
			if(action & RC_SERVICE_START) start_auto46det(safe_atoi(cmd[1]), safe_atoi(cmd[2]));
		else
			if(action & RC_SERVICE_START) start_auto46det(1, 0);
	}
	else if (strcmp(script, "v6plusd") == 0)
	{
		int unit = (cmd[1]) ? safe_atoi(cmd[1]) : wan_primary_ifunit();
	...
	{
		check_wifi_onoff(safe_atoi(cmd[1]));
	}
	else if (strcmp(script, "vpnctrl") == 0)
	{
		check_vpn_onoff(safe_atoi(cmd[1]));
	}
	else if (strcmp(script, "sdnctrl") == 0)
	{
		check_sdn_onoff(safe_atoi(cmd[1]));
	}
	else if (strcmp(script, "odctrl") == 0)
	{
		check_od_onoff(safe_atoi(cmd[1]));
	}
#endif	
...
// end of add
	...
	{
		int stop = (count > 1) ? safe_atoi(cmd[1]) : 1;
		dbg("stop: %d\n", stop);
	...
	else if (strcmp(script, "sched_daemon") == 0)
	{
		...
		if (cmd[1]) {
			if (cmd[2]) {
				int s_unit = safe_atoi(cmd[1]);
				int c_unit = safe_atoi(cmd[2]);
				update_wgs_client(s_unit, c_unit);
			}
			else {
				int c_unit = safe_atoi(cmd[1]);
	...
	{
		if (cmd[1])
			handle_ds_switch(safe_atoi(cmd[1]));	/* 1: parent, 2: child */
		else
			handle_ds_switch(0);
	}
