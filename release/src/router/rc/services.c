...
int mkdir_if_none(const char *path)
{
	...
	if (!(dp = opendir(path))) {
		...
		safe_do_system("mkdir -m 0777 -p '%s'", (char *)path);
		return 1;
	}
	closedir(dp);
...
#define FWUPDATE_DBG(fmt,args...) \
        do { \
                FILE *_dbg_fp = fopen("/tmp/webs_upgrade.log", "a"); \
                if (_dbg_fp) { \
                        fprintf(_dbg_fp, "[FWUPDATE][%s:(%d)]"fmt"\n", __FUNCTION__, __LINE__, ##args); \
                        fclose(_dbg_fp); \
                } \
        } while(0)
...
static int __cp(const char *param, const char *dir, const char *files, const char *newroot)
{
	...
	case 1:	/* copy a directory recursive */
		...
		safe_do_system("cp -af%s \"%s\" \"%s\"", param, dir, dest_dir);
...
#if defined(RTCONFIG_IPV6) && defined(RTCONFIG_MULTILAN_CFG) && defined(RTCONFIG_MULTIWAN_IF)
static int _write_dnsmasq_conf_mtwan6_lan_prefix(FILE* fp)
{
	MTLAN_T *pmtl = NULL;
	size_t mtl_sz = 0;
	int have_dhcp = 0;
	...
	get_mtlan_by_idx(SDNFT_TYPE_SDN, LAN_IN_SDN_IDX, pmtl, &mtl_sz);
	if (mtl_sz > 0)
		have_dhcp |= write_dnsmasq_conf_sdn_ra_dhcp6_range(fp, &pmtl[0]);
...
#endif
{
	...
	if (client_mode() && nvram_match("x_Setting", "1")) 
		return;
#endif
	TRACE_PT("begin\n");
...
//#endif
...
#ifdef RTCONFIG_NOWL
                || (!is_routing_enabled() && nvram_match("dhcp_enable_x2", "1"))
#endif
	) {
	...
	if (ipv6_enabled() && is_routing_enabled()) {
		...
		have_dhcp |= _write_dnsmasq_conf_mtwan6_lan_prefix(fp);
#else
	...
	if (pmtl) {
		...
		for (i = 1; i < mtl_sz; i++) {
			if (pmtl[i].enable && pmtl[i].nw_t.v6_enable && pmtl[i].nw_t.idx != 0) {
				wan6_unit = get_sdn_wan6_unit(&pmtl[i], &share_subnet);
				_dprintf("subnet: %d, wan6_unit: %d, share_subnet: %d\n", pmtl[i].nw_t.idx, wan6_unit, share_subnet);
				...
				memset(sdn_prefix, 0, sizeof(sdn_prefix));

				if (share_subnet && (v6_prefix_length <= 56 || v6_prefix_div)) {
					if (v6_prefix[0]) {
						sdn_prefix_length = mtlan_extend_prefix_by_subnet_idx(v6_prefix, v6_prefix_length, pmtl[i].nw_t.idx, 8, sdn_prefix, sizeof(sdn_prefix));
						...
						sdn_prefix_length = mtlan_extend_prefix_by_subnet_idx(v6_ula_random, 48, pmtl[i].nw_t.idx, 16, sdn_prefix, sizeof(sdn_prefix));
						if (pmtl[i].nw_t.addr6[0])
							snprintf(ip, sizeof(ip), "%s%s/%d", sdn_prefix, pmtl[i].nw_t.addr6, sdn_prefix_length);
						else
							snprintf(ip, sizeof(ip), "%s1/%d", sdn_prefix, sdn_prefix_length);
					...
					if (wan6_unit < WAN_UNIT_MAX)
						...
						cnt++;
					if (*sdn_prefix) {
						snprintf(nv, sizeof(nv), "ipv6_prefix_sdn%d", pmtl[i].sdn_t.sdn_idx);
						nvram_set(ipv6_nvname_by_unit(nv, wan6_unit), sdn_prefix);
					}
				}
#ifdef RTCONFIG_MULTIWAN_IF
				else if (share_subnet == 0 && wan6_unit != wan_primary_ifunit_ipv6()) {
					if (v6_rtr_addr[0]) {
				...
				p = strchr(ip, '/');
				*p = '\0';
				snprintf(nv, sizeof(nv), "ipv6_rtr_addr_sdn%d", pmtl[i].sdn_t.sdn_idx);
...
char* _add_ip6_lansdnaddr()
{
...
#ifdef RTCONFIG_MULTIWAN_IF
	wan6_unit = mtwan6_get_sdn_wan6_unit(LAN_IN_SDN_IDX);
	if (wan6_unit == WAN_UNIT_NONE) {
		_dprintf("%s:%d: No active wan6 unit\n", __FUNCTION__, __LINE__);
		return NULL;
	}

	if (wan6_unit != wan_primary_ifunit_ipv6()) {
		switch (get_ipv6_service_by_unit(wan6_unit)) {
		case IPV6_NATIVE_DHCP:
			if (nvram_get_int(ipv6_nvname_by_unit("ipv6_dhcp_pd", wan6_unit)))
				break;
			/* fall through */
#ifdef RTCONFIG_6RELAYD
...
#endif
		case IPV6_MANUAL:
			p = ipv6_prefix_by_unit(NULL, wan6_unit);
			if (*p && !nvram_match(ipv6_nvname_by_unit("ipv6_prefix", wan6_unit), (char*)p))
				nvram_set(ipv6_nvname_by_unit("ipv6_prefix", wan6_unit), (char*)p);
			break;
		}
	}
#endif
	...
	if (set_ip6_sdnaddr()) {
		...
		if (v6_prefix[0]) {
			...
			if (sdn_prefix_length) {
				snprintf(ip, sizeof(ip), "%s1/%d", sdn_prefix, sdn_prefix_length);
				nvram_set_int(ipv6_nvname_by_unit("ipv6_prefix_length_sdn", wan6_unit), sdn_prefix_length);
			}
			else if ((p = nvram_get(ipv6_nvname_by_unit("ipv6_rtr_addr", wan6_unit)))) {
		...
		p = strchr(ip, '/');
		*p = '\0';
		if (!nvram_match(ipv6_nvname_by_unit("ipv6_rtr_addr", wan6_unit), ip))
			nvram_set(ipv6_nvname_by_unit("ipv6_rtr_addr", wan6_unit), ip);
...
#ifdef RTCONFIG_MULTIWAN_IF
		p = (char*)ipv6_router_address_by_unit(NULL, wan6_unit);
#else
		...
		if (*p) {
			...
			if (!nvram_match(ipv6_nvname_by_unit("ipv6_rtr_addr", wan6_unit), (char*)p))
				nvram_set(ipv6_nvname_by_unit("ipv6_rtr_addr", wan6_unit), (char*)p);
		}
...
void add_ip6_lanaddr(void)
{
	...
	if (nvram_get_int("telnetd_enable") && *p) {
		...
		if (getpid() == 1) {
			system("service restart_telnetd &");
		}
		else
#endif
...
void start_s46_tunnel(int unit)
{
	...
	char ipaddr[INET_ADDRSTRLEN], AFTR[INET6_ADDRSTRLEN], local[INET6_ADDRSTRLEN];
	...
	case WAN_DSLITE:
		...
		if (get_ipv6_service_by_unit(unit) == IPV6_NATIVE_DHCP)
			get_s46_prefix_host(nvram_safe_get(ipv6_nvname_by_unit("ipv6_prefix", unit)), local, sizeof(local));
		else
			snprintf(local, sizeof(local), "%s", nvram_safe_get(ipv6_nvname_by_unit("ipv6_ra_addr", unit)));
		break;
	default:
		return;
	}
...
int no_need_to_start_wps(void)
{
	...
	if (nvram_match("wps_enable", "1") && nvram_match("re_mode", "0")
	    && sw_mode() == SW_MODE_AP 
	    && (   (nvram_match("wlc_psta", "0") && nvram_match("wlc_dpsta", "0")) 
	        || (nvram_match("wlc_psta", "2") && nvram_match("wlc_dpsta", "2") && nvram_match("mlo_mb", "0")))
	   )
		return 0;
#endif
...
start_ddns(char *caller, int isAidisk)
{
	...
	if (inet_addr_(wan_ip) == inet_addr_(nvram_safe_get("ddns_ipaddr")) &&
		...
		((strnlen(ip6_addr, INET6_ADDRSTRLEN) == 0 && strnlen(nvram_safe_get("ddns_ipv6_ipaddr"), INET6_ADDRSTRLEN) == 0) || nvram_match("ddns_ipv6_ipaddr", ip6_addr)))) && (strcmp(ipv6_service_cur, nvram_safe_get("ddns_ipv6_service_old")) == 0)
#endif
	...
	if (strcmp(ipv6_service_cur, nvram_safe_get("ddns_ipv6_service_old")) != 0){
		nvram_set("ddns_ipv6_service_old", ipv6_service_cur);
	}
#endif
...
#endif
{
	...
	fprintf(fp, "igmp-max-groups 128\n");
	fprintf(fp, "igmp-max-sources 128\n");
	fprintf(fp, "igmp-max-members 128\n");
...
int no_need_acsd()
{
	if (nvram_get_int("acs_disable"))
		return 1;

#ifdef RTAC68U
	if (is_dpsta_repeater() && !nvram_get_int("x_Setting"))
		return 1;
#elif defined(RTCONFIG_AMAS_5G_ONBOARDING)
...
start_acsd()
{
	...
	// If ciscan_enable is set, restart ciscan_monitor to make sure it works with the latest acs settings
	if (nvram_get_int("ciscan_enable")) {
		stop_ciscan_monitor();
		start_ciscan_monitor();
	}
#endif
...
#define AFC_ACTIVE_TS_DEBUG     "/tmp/afc_active_ts_debug"
#define AFC_MAX_TM              25
#define AFC_ONE_DAY             86400

static void afc_json_ExpireTime_string(char *file, char *buf, int len)
{
	json_object *root = NULL;
	json_object *availableSpectrumInquiryResponses;
	json_object *first_response;
	json_object *availabilityExpireTime;
	const char *tmpval;

	memset(buf, 0, len);
	if ((root = json_object_from_file(file)) == NULL) {
		goto END;
	}

	if (!json_object_object_get_ex(root, "availableSpectrumInquiryResponses", &availableSpectrumInquiryResponses)) {
		goto END;
	}

	if ((first_response = json_object_array_get_idx(availableSpectrumInquiryResponses, 0)) == NULL) {
		goto END;
	}

	if (!json_object_object_get_ex(first_response, "availabilityExpireTime", &availabilityExpireTime)) {
		goto END;
	}

	tmpval = json_object_get_string(availabilityExpireTime);
	AFC_DBG("DEBUG", "val=%s\n", tmpval);
	if (tmpval) snprintf(buf, len, "%s", tmpval);

END:
	if (root) json_object_put(root);
}

void get_afc_response_tstamp()
{
	time_t response_t;
	struct tm ts;
	char buf[AFC_MAX_TM] = {0};
	char log[128] = {0};

	afc_json_ExpireTime_string(AFC_RESPONSE_FILE, buf, sizeof(buf));
...
END:
	nvram_set_int("afc_active_ts", response_t);
	AFC_DBG("INFO", "response_t=%ld\n", response_t);

	if (nvram_get_int("afc_debug")) {
		safe_do_system_to_file(AFC_ACTIVE_TS_DEBUG, 0, "echo %ld");
	}
}
...
void locpold_config()
{
	nvram_set("locpold_cmd_path", "/var/afc/locpol_daemon");
	nvram_set("locpold_ipc_type", "unix_socket");
	nvram_set("locpold_mode", "Enabled");
	nvram_set("locpold_timeout", "300");
	nvram_set("mobappd_cmd_path", "/var/afc/mobile_app_daemon");
	nvram_set("mobappd_ipc_type", "unix_socket");
	nvram_set("mobappd_timeout", "300");
}

void clean_afc_power(void)
{
	char wlif[16] = {0};
	char buff[32] = {0};

	snprintf(wlif, sizeof(wlif), "wl%d_ifname", WL_6G_BAND);
	safe_do_system("wl -i %s afc_info -1", nvram_safe_get(wlif));
#ifdef RTCONFIG_HAS_6G_2
	snprintf(wlif, sizeof(wlif), "wl%d_ifname", WL_6G_2_BAND);
	safe_do_system("wl -i %s afc_info -1", nvram_safe_get(wlif));
#endif
}
...
start_bcm_afc(void)
{
	/* for heartbeat, we need to genearate certificate */
	if (IS_AFC_PP() == 1) {
		gen_afc_cert();
	}
...
{
	if (pidof("afcd") > 0)
		doSystem("rm /tmp/dm/%d", pidof("afcd"));
...
start_afcd(void)
{
	...
	char *afcd_argv[] = { "/usr/sbin/afcd", NULL };

	stop_afcd();
	ret = _eval(afcd_argv, NULL, 0, &pid);
...
{
	if (pidof("locpold") > 0)
		doSystem("rm /tmp/dm/%d", pidof("locpold"));
...
start_locpold(void)
{
	...
	char *locpold_argv[] = { "/usr/sbin/locpold", NULL };

	stop_locpold();
	ret = _eval(locpold_argv, NULL, 0, &pid);
...
#endif
#endif
#if defined(RTCONFIG_BRCM_SMARTHAUL) || defined(RTCONFIG_QOSR1) 
	stop_qosmgmtd();
#endif
	stop_acsd();
#if defined(RTCONFIG_BCMWL6) && defined(RTCONFIG_WISP)
...
#endif
#ifdef RTCONFIG_TUNNEL
	//stop_mastiff(); //pass kill tunnel for remote fw upgrade
#endif
#ifdef RTCONFIG_NFCM
	stop_nfcm();
#endif
...
#endif
#if defined(RTCONFIG_MULTILAN_CFG) && defined(RTCONFIG_NMP_MATTER)
	char apg[128] = {0}, *apg_next = NULL, interfaces[256] = {0};
	int i = 0;
	foreach(apg, nvram_safe_get("apg_ifnames"), apg_next) {
		if(i == 0) {
			strlcpy(interfaces, apg, sizeof(interfaces));
		} else {
			strlcat(interfaces, ",", sizeof(interfaces));
			strlcat(interfaces, apg, sizeof(interfaces));
		}
		i++;
	}
	fprintf(fp, "allow-interfaces=br0,%s\n", interfaces);
#endif
...
#if defined(RTCONFIG_BT_CONN) || (defined(RTCONFIG_MDNS) && defined(RTCONFIG_NMP_MATTER))
...
static void _get_ble_name(char* name, size_t len)
{
	...
	/* Bundle flag */
#if defined(RTAX95Q) || defined(XT8PRO) || defined(BT12) || defined(BT10) || defined(BQ16) || defined(BQ16_PRO) || defined(BN12) || defined(BM68) || defined(XT8_V2) || defined(RTAXE95Q) || defined(ET8PRO) || defined(ET8_V2) || defined(RTAX56_XD4) || defined(XD4PRO) || defined(RTAX82_XD6) || defined(RTAX82_XD6S) || defined(ET12) || defined(XT12) || defined(XD6_V2) || defined(XC5) || defined(EBG15) || defined(EBG19P)
...
#if defined(ET12) || defined(XT12) || defined(EBG15) || defined(EBG19P)
...
#if defined(RTAX95Q) || defined(XT8PRO) || defined(BT12) || defined(BT10) || defined(BQ16) || defined(BQ16_PRO) || defined(BN12) || defined(BM68) || defined(XT8_V2) || defined(RTAXE95Q) || defined(ET8PRO) || defined(ET8_V2) || defined(RTAX56_XD4) || defined(XD4PRO) || defined(RTAX82_XD6) || defined(RTAX82_XD6S) || defined(ET12) || defined(XT12) || defined(XD6_V2) || defined(BC109) || defined(EBG19P) || defined(BC105) || defined(EBG15) || defined(XC5)
...
#if !defined(BLUECAVE) && !defined(RTAX95Q) && !defined(XT8PRO) && !defined(BT12) && !defined(BT10) && !defined(BQ16) && !defined(BQ16_PRO) && !defined(BN12) && !defined(BM68) && !defined(XT8_V2) && !defined(RTAXE95Q) && !defined(ET8PRO) && !defined(ET8_V2) && !defined(RTAX56_XD4) && !defined(XD4PRO) && !defined(RTAX82_XD6) && !defined(RTAX82_XD6S) && !defined(ET12) && !defined(XT12) && !defined(XD6_V2) && !defined(BC109) && !defined(EBG19P) && !defined(EBG15) && !defined(BC105) && !defined(XC5)
...
unsigned long off_pd() {
	FILE *fp = NULL;
	char cmd[128], buf[128];
	unsigned long val = 0;

	snprintf(cmd, sizeof(cmd), "i2cget -y 0 0x21 0x20");
	fp = popen(cmd, "r");
	if(fp) {
		memset(buf, 0, sizeof(buf));
		if(fgets(buf, sizeof(buf), fp) != NULL) {
			...
			val |= 1<<5;
			_dprintf("%s, rewrite typec state: %lx\n", __func__, val);
			safe_do_system("i2cset -y 0 0x21 0x20 0x%lx", val);
		}
		pclose(fp);
	}
...
unsigned long pd_read(char *offset)
{
	FILE *fp = NULL;
	char cmd[128], buf[128];
	unsigned long val = 0;
	...
	if (!offset || !*offset) {
		_dprintf("%s: invalid offset\n", __func__);
		return 0;
	}
 
	snprintf(cmd, sizeof(cmd), "i2cget -y 0 0x21 %s", offset);
	//_dprintf("%s: [%s]\n", __func__, cmd);
	...
	_dprintf("%s: 0x%lx(%lu) units\n", __func__, val, val);
	return val;
}

/* 100mV/unit */
unsigned long pd_read_protocol_volt() 
{
	return pd_read("0x13") * 100;
}

/* 50mA/unit */
unsigned long pd_read_protocol_curr() 
{
	return pd_read("0x14") * 50;
}

/* 100mV/unit */
unsigned long pd_read_vbus_pwr() 
{
	return pd_read("0x15") * 100;
}

/* 100mV/unit */
unsigned long pd_read_vbus_mon() 
{
	return pd_read("0x16") * 100;
}

/* 50mA/unit */
unsigned long pd_read_typec_curr() 
{
	return pd_read("0x17") * 50;
}

int pd_dump()
{
	unsigned long pd_busv = 0;
	unsigned long pd_prot_v = 0;
	unsigned long pd_prot_cur = 0;
	unsigned long pd_bus_pwr = 0;
	unsigned long pd_typec_cur = 0;
	int pd_low = 0;

	printf("\n\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n");
	pd_busv = pd_read_vbus_mon();
	_dprintf("%s, (0x16) pd_busv=%lu\n", __func__, pd_busv);

	pd_prot_v = pd_read_protocol_volt();
	_dprintf("%s, (0x13) pd_read_protocol_volt=%lu\n", __func__, pd_prot_v);

	pd_prot_cur = pd_read_protocol_curr();
	_dprintf("%s, (0x14) pd_read_protocol_curr=%lu\n", __func__, pd_prot_cur);

	pd_bus_pwr = pd_read_vbus_pwr();
	_dprintf("%s, (0x15) pd_read_vbus_pwr=%lu\n", __func__, pd_bus_pwr);

	pd_typec_cur = pd_read_typec_curr();
	_dprintf("%s, (0x17) pd_read_typec_curr=%lu\n", __func__, pd_typec_cur);

	printf("\n<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n\n");

	nvram_set_int("pd_busv", pd_busv);
	nvram_set_int("pd_protv", pd_prot_v);
	nvram_set_int("pd_protc", pd_prot_cur);
	nvram_set_int("pd_busp", pd_bus_pwr);
	nvram_set_int("pd_typec", pd_typec_cur);

	if (pd_busv < 5000)
		pd_low = 2;
	else if (pd_busv < 9000)
		pd_low = 1;
...
unsigned long pd_loading_test() 
{
	int pd_low = 0;
	pthread_t threads[NUM_THREADS];
	int run_seconds = nvram_get_int("pdtest_run")?:RUN_SECONDS;
	int num_threads = nvram_get_int("pdtest_threads")?:NUM_THREADS;

	printf("%s: Starting CPU load for %d seconds using %d threads...\n", __func__, run_seconds, num_threads);
	pd_dump();
	...
	// Sleep while CPU is loaded
	printf("%s, burn cpu and chk pd status..\n", __func__);
	sleep(run_seconds);
	pd_low = pd_dump();
	...
	printf("CPU load finished. Check power readings from PD chip.\n");
...
void radiusd_updateDB(void)
{
		...
		/* create database schema — .read is a sqlite3 CLI command, requires shell */
		...
		if (sqlite3_open("/tmp/freeradius.db", &rdb) != SQLITE_OK) {
			logmessage("radiusd", "radiusd_updateDB: failed to open freeradius.db");
			return;
		}
		...
		if (nv) {
			while ((b = strsep(&nvp, "<")) != NULL) {
				...
				while((tmp = strsep(&userlist, ",")) != NULL ){
					...
					sscanf(tmp, "%127[^:]:%127s", name, passwd);

					rrc = sqlite3_prepare_v2(rdb,
						"INSERT INTO radcheck(username,attribute,op,value) VALUES(?,?,?,?)",
						-1, &rstmt, NULL);
					if (rrc == SQLITE_OK) {
						sqlite3_bind_text(rstmt, 1, name,   -1, SQLITE_STATIC);
						sqlite3_bind_text(rstmt, 2, "Cleartext-Password", -1, SQLITE_STATIC);
						sqlite3_bind_text(rstmt, 3, ":=",   -1, SQLITE_STATIC);
						sqlite3_bind_text(rstmt, 4, passwd, -1, SQLITE_STATIC);
						sqlite3_step(rstmt);
					...
					if (rrc == SQLITE_OK) {
						sqlite3_bind_text(rstmt, 1, name,     -1, SQLITE_STATIC);
						sqlite3_bind_text(rstmt, 2, "NAS-Identifier", -1, SQLITE_STATIC);
						sqlite3_bind_text(rstmt, 3, "==",     -1, SQLITE_STATIC);
						sqlite3_bind_text(rstmt, 4, "radius", -1, SQLITE_STATIC);
						sqlite3_step(rstmt);
						sqlite3_finalize(rstmt);
						rstmt = NULL;
					}
		...
		sqlite3_exec(rdb, "DELETE FROM nas", NULL, NULL, &rErrMsg);
		if (rErrMsg) { sqlite3_free(rErrMsg); rErrMsg = NULL; }
		...
		if (nv) {
			while ((b = strsep(&nvp, "<")) != NULL) {
				...
				if(atoi(cli_act) == 1){
					rrc = sqlite3_prepare_v2(rdb,
						"INSERT INTO nas(id,nasname,shortname,secret) VALUES(NULL,?,?,?)",
						-1, &rstmt, NULL);
					if (rrc == SQLITE_OK) {
						sqlite3_bind_text(rstmt, 1, cli_ip,     -1, SQLITE_STATIC);
						sqlite3_bind_text(rstmt, 2, cli_name,   -1, SQLITE_STATIC);
						sqlite3_bind_text(rstmt, 3, cli_passwd, -1, SQLITE_STATIC);
						sqlite3_step(rstmt);
						sqlite3_finalize(rstmt);
						rstmt = NULL;
					}
...
void start_poemon()
{
        char *argv[] = {"poemon", NULL};

        pid_t pid;

        stop_poemon();

        if (nvram_match("disable_poeAlert", "1") || nvram_match("no_pse", "1"))
                return;
...
void restart_poeInit()
{
        char *argv[] = {"poe_init", NULL};
...
void start_CP(void)
{
	...
	if(cp_local_auth){
		...
		memset(cr_en,0,256);
		memset(cr,0,256);
		snprintf(cr_en,256,nvram_safe_get(bn));
		int cr_len = strlen(cr_en);

		if(strstr(cr_en,">") || strstr(cr_en,"<")){
			base64_encode((unsigned char *)cr_en,cr,cr_len);
			cr[strlen(cr)+1] = '\0';
		}
		else {
			snprintf(cr,256,nvram_safe_get(bn));
	...
	char *nv=NULL, *nvp=NULL, *b=NULL;
	int brCount=0;
	char *profileName_en=NULL, *htmlIdx=NULL,    *awayTime=NULL,   *sessionTime=NULL;
	...
	nv = nvp = strdup(nvram_safe_get("captive_portal_adv_profile"));
        if (nv) {
		   ...
		   chmod("/tmp/uamsrv/www", 0777);
                   ...
                   while ((b = strsep(&nvp, "<")) != NULL) {
                          if ((vstrsep(b, ">", &htmlIdx, &profileName_en, &awayTime, &sessionTime,
		...
		if(strstr(cr_en,"<") || strstr(cr_en,">")){
			base64_decode(profileName_en,profileName,strlen(profileName_en));
			profileName[strlen(profileName)+1] = '\0';
		}
		else{
			snprintf(profileName,strlen(profileName_en)+1,profileName_en);
		}
#ifdef	RTCONFIG_MULTILAN_CFG
#else
...
*/
	...
	char *UIType=NULL, *UIPath=NULL, *enService=NULL, *brdName_en=NULL, *enPass;
	char brdName[256];
	...
	nv = nvp = strdup(nvram_safe_get("captive_portal"));
        if (nv) {
		   ...
		   chmod("/tmp/uamsrv/www", 0777);
              ...
              while ((b = strsep(&nvp, "<")) != NULL) {
                    if ((vstrsep(b, ">", &htmlIdx, &brdName_en, &sessionTime, &UIType, &UIPath, &ifName, &enService, &enPass, &bandwidthMaxDown, &bandwidthMaxUp) < 9 ))
                        continue;
			  brCount++;

		if(strstr(cr_en,"<") || strstr(cr_en,">")){
			base64_decode(brdName_en,brdName,strlen(brdName_en));
			brdName[strlen(brdName)+1] = '\0';
		}
		else{
			snprintf(brdName,strlen(brdName_en)+1,brdName_en);
		}
...
stop_services(void)
{
 ...
 ||  (defined(RTCONFIG_SOC_IPQ8074))
	...
	stop_antled();
#endif
#if defined(RTAX82U) || defined(DSL_AX82U) || defined(GSAX3000) || defined(GSAX5400) || defined(TUFAX5400) || defined(GTAX11000_PRO) || defined(GTAXE16000) || defined(GTBE98) || defined(GTBE98_PRO) || defined(GTAX6000) || defined(GT10) || defined(RTAX82U_V2) || defined(TUFAX5400_V2) || defined(GTBE96) || defined(GTBE19000) || defined(GTBE19000AI) || defined(GSiBE18000) || defined(GSBE12000) || defined(GS7_PRO) || defined(GT7) || defined(GS7_PRO_MAX) || defined(GTBE96_AI) || defined(RTCONFIG_AURALED)
	stop_ledg();
	stop_ledbtn();
#endif
#ifdef RTCONFIG_TCPLUGIN
...
start_sw_devled(void)
{
	...
	return;
#endif
#if defined(RTAX95Q) || defined(XT8PRO) || defined(BT12) || defined(BT10) || defined(BQ16) || defined(BQ16_PRO) || defined(BN12) || defined(BM68) || defined(XT8_V2) || defined(RTAXE95Q) || defined(ET8PRO) || defined(ET8_V2) || defined(RTAX56U) || defined(RTAX56_XD4) || defined(XD4PRO) || defined(CTAX56_XD4) || defined(ET12) || defined(XT12) || defined(XC5) || defined(EBA63)
	return 1;
#endif
...
#if defined(RTCONFIG_SW_BTN)
int stop_sw_btn(void)
{
	if (pids("sw_btnd"))
		killall_tk("sw_btnd");
	return 0;
}	
int start_sw_btn(void)
{
	...
	if(nvram_match("x_Setting", "1")) { // sync btnsw_onoff & default new image if needed
		...
		if (nvram_get_int("btnsw_onoff") == 1) {
			unsigned char comment_buf[50], modes[5];
			int ret;
			ret = nvsw_get_info(1, comment_buf, sizeof(comment_buf), modes, sizeof(modes));
			if (ret) {// not yet initialize
				nvsw_def_nvimg(1, "Travel");
			}
		}
		nvsw_set(1, nvram_get_int("btnsw_onoff"));
#else
		nvsw_set(nvram_get_int("nvsw_tid"), nvram_get_int("btnsw_onoff"));
#endif
	}
#endif
	return _eval(swbtn_argv, NULL, 0, &pid);
}	
...
#ifdef RTCONFIG_MXL_826XX
int
stop_mxlmonitor(void)
{
	if (pids("mxlmonitor"))
		killall_tk("mxlmonitor");

	return 0;
}

int
start_mxlmonitor(void)
{
	char *mxlmonitor_argv[] = {"mxlmonitor", NULL};
	pid_t pid;
	...
	if (is_router_mode())
#endif
	return _eval(mxlmonitor_argv, NULL, 0, &pid);
}
#endif
...
{
        if (pids("tempsense"))
                killall_tk("tempsense");

        return 0;
}

int
start_tempsense(void)
{
        char *tempsense_argv[] = {"tempsense", NULL};
        pid_t pid;

        stop_tempsense();

        return _eval(tempsense_argv, NULL, 0, &pid);
}
#endif
...
start_ext84991(void)
{
	char *ext84991_argv[] = {"ext84991", NULL};
	pid_t pid;

	stop_ext84991();

	return _eval(ext84991_argv, NULL, 0, &pid);
}
...
#endif
#ifdef RTCONFIG_OPENVPN
#if defined(RTCONFIG_UBIFS) || defined(RTCONFIG_JFFS_PARTITION)
...
void ai_init_default_hostname(void)
{
	char ai_mac_s[32] = {0}, ai_mac_suffix[8] = {0}, ai_hostname_s[64] = {0};
	int suffix_idx = 0, mac_idx = 0, ai_cmd_ret = 1;
...
void ai_sync_reset (int send_apply_req)
{
	char enc_passwd[NVRAM_ENC_MAXLEN];

	// AI_SET_REQ
	if (nvram_default_get("ai_ssh_enable"))
		nvram_set("ai_ssh_enable", nvram_default_get("ai_ssh_enable"));
	if (nvram_default_get("ai_ssh_port"))
		nvram_set("ai_ssh_port", nvram_default_get("ai_ssh_port"));
	nvram_unset("ai_ssh_disallow_root_logins");
	if (nvram_default_get("ai_ssh_acc"))
		nvram_set("ai_ssh_acc", nvram_default_get("ai_ssh_acc"));
	if (nvram_default_get("ai_ssh_pass")) {
		memset(enc_passwd, 0, sizeof(enc_passwd));
		pw_enc(nvram_default_get("ai_ssh_pass"), enc_passwd, 1);
		nvram_set("ai_ssh_pass", enc_passwd);
	}
	if (nvram_default_get("ai_lan_proto"))
		nvram_set("ai_lan_proto", nvram_default_get("ai_lan_proto"));
	if (nvram_default_get("ai_lan_ip"))
		nvram_set("ai_lan_ip", nvram_default_get("ai_lan_ip"));
	if (nvram_default_get("ai_lan_netmask"))
		nvram_set("ai_lan_netmask", nvram_default_get("ai_lan_netmask"));
	if (nvram_default_get("ai_lan_gateway"))
		nvram_set("ai_lan_gateway", nvram_default_get("ai_lan_gateway"));
	if (nvram_default_get("ai_dns"))
		nvram_set("ai_dns", nvram_default_get("ai_dns"));
	
	if (nvram_default_get("ai_portainer_enable"))
		nvram_set("ai_portainer_enable", nvram_default_get("ai_portainer_enable"));
	if (nvram_default_get("ai_portainer_acc"))
		nvram_set("ai_portainer_acc", nvram_default_get("ai_portainer_acc"));
	if (nvram_default_get("ai_portainer_pass")) {
		memset(enc_passwd, 0, sizeof(enc_passwd));
		pw_enc(nvram_default_get("ai_portainer_pass"), enc_passwd, 1);
		nvram_set("ai_portainer_pass", enc_passwd);
	}
	ai_init_default_hostname();
	nvram_unset("ai_app_config_renew");
	nvram_unset("ai_response_debug");
	if(send_apply_req) send_ai_request(AI_SET_REQ);
}
#endif	/* RTCONFIG_AI_SERVICE */
...
#endif
#if defined(RTCONFIG_AI_SERVICE)
		_dprintf("%s: Do AI sync reset\n", __FUNCTION__);
		ai_sync_reset(1);
		system("rm -rf /ai/.diag");
#endif
		factory_reset();
	}
#if 0
...
#if 1 // PM regret
                if (nvram_get("nvswid")) {
                        nvsw_def_nvimg(nvram_get_int("nvswid"), NULL);
                }
#endif
#endif
#if defined(RTCONFIG_AI_SERVICE)
...
/* TODO fix fini_wl() for BCM USBAP */
		...
		modprobe_r("udf");
#endif
#if (defined(RTCONFIG_TFAT) || defined(RTCONFIG_TUXERA_NTFS) || defined(RTCONFIG_TUXERA_HFS)) && (defined(WIFI7_SDK_20250506) || defined(WIFI8_SDK_20251126) || defined(WIFI8_SDK_20260204) || defined(WIFI8_SDK_20260402) || defined(GSBE18000) || defined(GSBE12000) || defined(GS7_PRO) || defined(GT7) || defined(GS7_PRO_MAX))
...
script_allnet:
	...
	script_net_and_phy:
		if(action & RC_SERVICE_STOP) {
			...
			stop_moca();
#endif
#if defined(RTCONFIG_MULTIWAN_PROFILE) && defined(BCM4906_504)
			init_switch();
#endif
			//stop_vlan();
			stop_lan_port();
...
#ifdef RTCONFIG_MULTIWAN_PROFILE
					handle_sdn_routing_ipv6_neigh(wan_unit, NULL);
#endif
#else
					start_lan_ipv6();
#endif	//RTCONFIG_MULTIWAN_IF
...
//		if(action & RC_SERVICE_STOP) stop_moca_mps();
...
#endif
#ifdef RTCONFIG_AI_SERVICE
	else if (strcmp(script, "ai_reboot") == 0)
	{
		if(action & RC_SERVICE_START) start_ai_reboot();
	}
	else if (strcmp(script, "ai_upgrade") == 0)
	{
		if(is_ai_if_on())
		{
			// for manually upload -> upgrade
			if(count > 1){
				if(action & RC_SERVICE_START){ 
					char filename[128] = {0};
					memset(filename, 0, sizeof(filename));
					snprintf(filename, sizeof(filename), "firmware/%s", cmd[1]);
	...
	else if (strcmp(script, "ai_service") == 0)
	{
		if(action & RC_SERVICE_STOP) stop_ai_service();
		if(action & RC_SERVICE_START) start_ai_service();
	}
	else if (strcmp(script, "ai_response_check") == 0)
	{
	...
	else if (strcmp(script, "ai_tftpd") == 0)
	{
		...
		if(action & RC_SERVICE_START) start_ai_tftpd();
	}
	else if (strcmp(script, "ai_request") == 0)
	{
		...
		if(is_ai_if_on())
		{
        		if(action & RC_SERVICE_START)
			{
				...
				{
					pid_t pid;
					char *cmds[4] = { script, NULL, NULL, NULL };
					for(int cmd_idx = 1; cmd_idx < count; cmd_idx++)
					{
						cmds[cmd_idx] = cmd[cmd_idx];
					}
					_eval(cmds, NULL, 0, &pid);
				}
        		}
		}
	}
...
#endif
#ifdef RTCONFIG_BCM_AFC
	else if (strcmp(script, "afcd_harness") == 0)
	{
		/*
		 * cmd[1] = 1, formal
		 * cmd[1] = 2, TestHarness
		 * cmd[1] = 3, WFA testing server
		 * */
		stop_afcd();
		afcd_harness_config(atoi(cmd[1]));
		start_afcd();
	}
	else if (strcmp(script, "afcd") == 0)
	{
		if(action & RC_SERVICE_STOP) {
			stop_afcd();
		}
	...
	else if (strcmp(script, "locpold") == 0)
	{
		if(action & RC_SERVICE_STOP) {
			stop_locpold();
		}
	...
	else if (strcmp(script, "send_afc_slient_notify") == 0)
	{
		send_afc_slient_notify();
	}
	else if (strcmp(script, "send_afc_notify") == 0)
	{
		send_afc_notify();
	}
	else if (strcmp(script, "afc_active_ts") == 0)
	{
		get_afc_response_tstamp();
	}
	else if (strcmp(script, "bcm_afc") == 0)
	{
		if(action & RC_SERVICE_STOP) {
			stop_bcm_afc();
		}
		...
		// trigger heartbeat from service
		kill_pidfile_s("/var/run/afc_heartbeat.pid", SIGUSR1);

#if defined(RTCONFIG_AMAS) && defined(RTCONFIG_CFGSYNC)
		if (is_cap())
			send_event_to_cfgmnt(EID_RC_RESTART_AFC);
#endif
	}
	else if (strcmp(script, "afc_info") == 0)
	{
		char *ifname = NULL;
		char *chanspec = NULL;
		if (cmd[1]) ifname = cmd[1];
		if (cmd[2]) chanspec = cmd[2];
		wl_get_afc_info(ifname, chanspec);
	}
	else if (strcmp(script, "afc_status") == 0)
	{
		wl_afc_status();
	}
	else if (strcmp(script, "afc_MeshPathLoss") == 0)
	{
		int rssi = atoi(cmd[1]);
		int tx = atoi(cmd[2]);
		int channel = atoi(cmd[3]);
		int band = atoi(cmd[4]);
...
//#ifdef RTCONFIG_CLOUDSYNC
	...
	else if (strcmp(script, "stuck") == 0)
	{
		if(*nvram_safe_get("stuck_time")) {
			_dprintf("dut would be stuck for %d seconds\n", nvram_get_int("stuck_time"));
			sleep(nvram_get_int("stuck_time"));
			_dprintf("stuck fin.\n");
		}
	}
#ifdef RTCONFIG_AUPNPC
	...
	else if (strcmp(script, "ark_force") == 0)
	{
		if (action & RC_SERVICE_STOP) stop_ark_engine(1);
	}
	else if (strcmp(script, "ark_qoe") == 0)
	{
		exe_ark_qoe_service();
	}
	else if (strcmp(script, "ark_iam") == 0)
	{
		nvram_set("restart_iam", "1");
		exe_ark_iam_service();
		nvram_set("restart_iam", "0");
	}
	else if (strcmp(script, "ark_limiter") == 0)
	{
		exe_ark_limiter_service();
	}
	else if (strcmp(script, "ark_sig_check") == 0)
	{
		if (action & RC_SERVICE_START) ark_sig_update_flow();
	}
	else if (strcmp(script, "ark_his_write") == 0)
	{
		exe_ark_history();
	}
	else if (strcmp(script, "ark_mon_write") == 0)
	{
		exe_ark_protection();
	}
	else if (strcmp(script, "ark_migrate") == 0)
	{
		exe_ark_migrate_run();
	}
#endif
#if defined(RTCONFIG_HNS)
	else if (strcmp(script, "hns_his_write") == 0)
	{
		exe_hns_history();
	}
	else if (strcmp(script, "hns_protection_write") == 0)
	{
		exe_hns_protection();
	}
	else if (strcmp(script, "hns_firewall") == 0)
	{
		add_hns_iptables_rules();
	}
	else if (strcmp(script, "wrs") == 0)
	{
		if (action & RC_SERVICE_STOP) stop_hns_engine();
		if (action & RC_SERVICE_START) start_hns_engine();
	}
	else if (strcmp(script, "sig_check") == 0)
	{
		if (action & RC_SERVICE_START) hns_sig_update_flow();
	}
	...
	else if (strcmp(script, "cc_event") == 0)
	{
...
#ifdef RTCONFIG_AI_SERVICE
			pid_t tz_pid;
			nvram_set("ai_app_name", APP_TZ);
			char *tz_sync_cmd[] = {"ai_request", "app", NULL};
			_eval(tz_sync_cmd, NULL, 0, &tz_pid);
#endif
		}
	}
	...
	else if (strcmp(script, "wps_enr")==0)
	{
		if (is_router_mode()
...
#endif
			|| (rp_mode() && nvram_get_int("re_mode") == 0)
#elif defined(RPAX58) || defined(RPBE58) || defined(RTBE58_GO)
			|| ((nvram_match("x_Setting", "0") || dpsr_mode() || nvram_match("wps_enable", "1")) && nvram_get_int("re_mode") == 0)
#endif
		) {
				int unit = nvram_get_int("wps_band_x");
			...
			if (
				...
				nvram_match("x_Setting", "0") || nvram_match("wps_enable", "1")
#else
				is_router_mode()
#endif
			) {
#if defined(RPAX56) || defined(RPAX58) || defined(RPBE58)
				nvram_set("wlc0_auth_mode", "psk2");
				nvram_set("wlc1_auth_mode", "psk2");
				nvram_set("wlc0_crypto", "aes");
				nvram_set("wlc1_crypto", "aes");
#endif
...
retry_wps_enr:
 ...
 && !defined(RTCONFIG_CONCURRENTREPEATER)
	...
	else if (strcmp(script, "wpasupp_hapd")==0)
	{
		if (action & RC_SERVICE_STOP)
		{
			if (!nvram_match("hapd_enable", "0")) {
	...
	else if (strcmp(script, "wpasupp_qis")==0)	// rp qis dedicated preauth
	{
		...
		char reason_tmp[16];
		int is_client_mode = client_mode();
		int reload_wait = nvram_get_int("reload_wait")?:5;
		int pre_auth = (nvram_match("x_Setting", "0") || is_client_mode) && !nvram_match("stop_preauth", "1");

		if(cmd[1])
			band = atoi(cmd[1]);

		_dprintf("%s wpasupp_qis..pre_auth:%d, wl%d, cli:%d(%d/%d), reload_wait:%d\n", __func__, pre_auth, band, is_client_mode, dpsta_mode(), dpsr_mode(), reload_wait);
	...
	else if (strcmp(script, "wpasupp")==0)	// warp ver by wpa_cli
	{
		...
		if(action & RC_SERVICE_START)
		{
			...
			snprintf(ifname, sizeof(ifname), "wl%d", unit);
			snprintf(prefix, sizeof(prefix), "wlc_");
			snprintf(prefix2, sizeof(prefix2), "wl%d_", unit);

			_dprintf("wlc_band = %d, ifname=%s\n", unit, ifname);

			nvram_set("apply_wlc", "1");
			generate_wl_para(ifname, unit, -1);
			
			_dprintf("chks: ssid	   : %s = %s\n", strcat_r(prefix2, "ssid", tmp2), nvram_safe_get(strcat_r(prefix2, "ssid", tmp)));
			_dprintf("chks: akm 	   : %s = %s\n", strcat_r(prefix2, "akm", tmp2), nvram_safe_get(strcat_r(prefix2, "akm", tmp)));
			_dprintf("chks: wpa_psk	   : %s = %s\n", strcat_r(prefix2, "wpa_psk", tmp2), nvram_safe_get(strcat_r(prefix2, "wpa_psk", tmp)));
			_dprintf("chks: crypto     : %s = %s\n", strcat_r(prefix2, "crypto", tmp2), nvram_safe_get(strcat_r(prefix2, "crypto", tmp)));
			_dprintf("chks: auth_mode_x: %s = %s\n", strcat_r(prefix2, "auth_mode_x", tmp2), nvram_safe_get(strcat_r(prefix2, "auth_mode_x", tmp)));
	...
	else if (strcmp(script, "afc_pos_req") == 0)
	{
		afc_main();
	}
	else if (strcmp(script, "afc_positioning") == 0)
	{
		if (action & RC_SERVICE_STOP) {
			stop_afc_positioning();
		}
		if (action & RC_SERVICE_START) {
			start_afc_positioning(cmd[1]); // default is silent mode
		}
	}
#endif /* RTCONFIG_AFC_POSITIONING */
			...
			if(nvram_match("vpnc_proto", "openvpn")){
				...
				while (nv && (b = strsep(&nvp, "<")) != NULL) {
					...
					if (is_tpvpn_configured(TPVPN_HMA, region, conntype, unit))
						...
						safe_fork_do_system("hmavpn setconf \"%s\" \"%s\" %d", region, conntype, unit);
					}
					break;
				}
				free(nv);
...
_dprintf("test 2. turn off the USB power during %d seconds.\n", reset_seconds[reset_num]);
	...
	else if (strcmp(script, "cfgnewob")==0)
	{
		if(action&RC_SERVICE_STOP) stop_cfgnewob();
		if(action&RC_SERVICE_START) start_cfgnewob();
	}
#endif
	...
	else if (strcmp(script, "clean_backup_log") == 0)
	{
        ...
        {
#if !(defined(GTBE98) || defined(GTBE98_PRO) || defined(GTBE96) || defined(GTBE19000) || defined(RTBE92U) || defined(RTBE95U) || defined(RTBE82U) || defined(TUFBE82) || defined(RTBE58U_PRO) || defined(RTCONFIG_MXL_826XX) || defined(GTBE19000AI) || defined(GTBE96_AI))
		if(!nvram_match("x_Setting", "0")) {
	...
	else if (strcmp(script, "ctrl_led") == 0)
	{
		...
		/* on/brightness */
			...
			if (AllLED == 0) {
				...
				if (nvram_get_int("single_led_night_mode_sched") >= 0)
		...
		if (led_val) {
			esr_set_led(ES_STS_ON, "SW_LED");
			esr_set_aura(nvram_get_int("ledg_night_mode")? ES_STS_HALF : ES_STS_ON, "SW_LED");
		}
		else {
			esr_set_led(ES_STS_OFF, "SW_LED");
			esr_set_aura(ES_STS_OFF, "SW_LED");
		}
#endif
	}
#endif
#if defined(RTCONFIG_SW_BTN)
	else if (strcmp(script, "swbtn") == 0)
	{
		if (action & RC_SERVICE_STOP) stop_sw_btn();
		if (action & RC_SERVICE_START) start_sw_btn();
	}
	else if (strcmp(script, "ledctrl") == 0)
	{
		if(strcmp(cmd[1], "0")==0) 
		{
			nvram_set("led_val","0");
			...
			set_rgbled(RGBLED_OFF);
#endif
		}	
		else if(strcmp(cmd[1], "1")==0) 
		{
			nvram_set("led_val","1");
			nvram_set("AllLED", "1");
#if defined(RTCONFIG_ZENWIFI_RGBLED)
			...
			_dprintf("\n ledctrl parameter error\n");

	}
	else if (strcmp(script, "wifictrl") == 0)
	{
		check_wifi_onoff(atoi(cmd[1]));
	}
	else if (strcmp(script, "vpnctrl") == 0)
	{
		check_vpn_onoff(atoi(cmd[1]));
	}
	else if (strcmp(script, "sdnctrl") == 0)
	{
...
// end of add
	...
	else if (strcmp(script, "mlo_down_up") == 0)
	{
		mlo_down_up();
	}
	else if (strcmp(script, "wl_down_up") == 0)
	{
		wl_down_up();
	}
#endif
		...
		if (action & RC_SERVICE_START) {
			nvram_set_int("stop_watchdog", 0);
			start_watchdog();
		}
	}
#endif
#ifdef RTCONFIG_ENERGY_SAVE
	else if(!strcmp(script, "esd")){
		if(action & RC_SERVICE_STOP){
			stop_esd();
		}
		if(action & RC_SERVICE_START){
	...
	else if (!strcmp(script, "es_check")) {
		esr_set_check();
	}
	...
	else if (strcmp(script, "dns_ping") == 0)
	{
		...
		if(action&RC_SERVICE_START) start_airiq_monitor();
	}
#endif
#ifdef RTCONFIG_CSIMON
...
#endif
#ifdef RTCONFIG_CISCAN_MONITOR
	else if (strcmp(script,"ciscan_monitor") == 0){
		if(action&RC_SERVICE_STOP) stop_ciscan_monitor();
		if(action&RC_SERVICE_START) start_ciscan_monitor();
	}
#endif
	else
	{
...
void mlo_down_up()
{
	_dprintf("[%s] Tigger MLO_DOWN_UP workaround\n", __func__);
	logmessage("MLO_DOWN_UP_WAR", "[%s] Tigger MLO_DOWN_UP workaround\n",  __func__);
	mlo_rescue_war();
}

void wl_down_up()
{
	_dprintf("[%s] Tigger WL_DOWN_UP workaround\n", __func__);
	logmessage("WL_DOWN_UP_WAR", "[%s] Tigger WL_DOWN_UP workaround\n",  __func__);
	system("wl --all down");
	sleep(2);
	system("wl --all up");
}
...
/* TODO: clarify do we need to use lan_hostname instead of productid here */
	...
	if(nvram_get_int("lldpd_dbg") == 1) {
		dbG("exec lldpd debug mode: lldpd -L /usr/sbin/lldpcli -s %s -dddd\n", productid);
		safe_fork_do_system("lldpd -L /usr/sbin/lldpcli -s \"%s\" -dddd", productid);
	}
	else
...
#ifdef RTCONFIG_AI_SERVICE
		case 3:
			_dprintf("rsa check ai file : %s | rsa file : %s", argv[1], argv[2]);

			if(check_ai_rsasign(argv[1], argv[2])) {
				_dprintf("AI rsasign %s check %s OK ", argv[1], argv[2]);
				nvram_set("ai_rsasign_check", "1");
			}
			else {
				_dprintf("AI rsasign %s check %s Fail", argv[1], argv[2]);
				nvram_set("ai_rsasign_check", "0");
			}
			break;
#endif
		case 2:
			...
			break;
		default:
			_dprintf("rsa sign check parameters number error [%d]\n", argc);
			return -1;
	}
	return 0;
}
...
#if defined(RTCONFIG_HNS)
void hns_sig_update_flow()
{
	sig_update_and_rsa_check();
	if (nvram_get_int("bwdpi_rsa_check") == 1 && nvram_get_int("sig_update_update") == 0 && nvram_get_int("sig_state_flag") == 1) {
		stop_hns_engine();
		start_hns_engine();
		nvram_set("sig_state_flag", "0");
	}
}
#endif
...
void sig_update_and_rsa_check()
{
	char *sig_update_argv[] = {"sig_update.sh", NULL};
	char *sig_upgrade_argv[] = {"sig_upgrade.sh", NULL};
	// update signature info
	_eval(sig_update_argv, NULL, 10, NULL);
	if (nvram_get_int("sig_state_flag")) {
		// download and rsa check signature
		_eval(sig_upgrade_argv, NULL, 10, NULL);
	}
}
#endif
...
int check_rsa_signature(char *fname, char *fsign, char *fpkey)
{
	...
	/* check file */
#ifdef RTCONFIG_AI_SERVICE
	if ((dataFileFP = fopen64(fname, "r")) == NULL) {
		_dprintf("Open64 dataFileFP failure\n");
		goto exit;
	}
#else
...
#endif
#ifdef RTCONFIG_AI_SERVICE
int check_ai_rsasign(char *fname, char *rsa_fname)
{
	return check_rsa_signature(fname, rsa_fname, "/usr/sbin/public.pem");
}
#endif
...
#ifdef RTCONFIG_AMAS_NEWOB
int start_cfgnewob(void)
{
	char *cfg_newob_argv[] = {"cfg_newob", NULL};
	pid_t pid;
	int ret = 0, sw_mode;

	sw_mode = sw_mode();
	if ((((sw_mode == SW_MODE_ROUTER) && (nvram_get_int("x_Setting") == 0)))
#if defined(RTCONFIG_AMAS_NEWOB_RP)
		|| (sw_mode == SW_MODE_REPEATER)
#endif
#if defined(RTCONFIG_AMAS_NEWOB_ROUTER_AP)
		|| ((nvram_get_int("re_mode") == 0) && (sw_mode == SW_MODE_AP)&& (nvram_get_int("cfg_recount") == 0))
		|| (((sw_mode == SW_MODE_ROUTER) && (nvram_get_int("x_Setting") == 1)) && (nvram_get_int("cfg_recount") == 0))
#endif
		)
	{
		if (sw_mode == SW_MODE_AP)
		{
			if (nvram_get_int("lan_state_t") != LAN_STATE_CONNECTED)
				return ret;
		}
		...
		if (pids("cfg_newob"))
		{
			killall_tk("cfg_newob");
		}
		ret = _eval(cfg_newob_argv, NULL, 0, &pid);
	}
	return ret;
}

void stop_cfgnewob(void)
{
	if (pids("cfg_newob"))
		killall_tk("cfg_newob");
}
...
void stop_airiq_monitor(void){
	if (!is_support_airiq())
		return;

	if(pids("airiq_monitor"))
		killall_tk("airiq_monitor");
}
void start_airiq_monitor(void){
	char *cmd[] = {"airiq_monitor", NULL};
	pid_t pid;
...
#if defined(RTCONFIG_SW_BTN)
int get_vpn_idx(int vid)
{	
	char *nv = NULL, *nvp = NULL, *b = NULL;
	char *desc = NULL, *proto = NULL, *server = NULL, *username = NULL, *passwd = NULL, *active = NULL, *idx = NULL;
        int ret = 0;
		...
		while (nv && (b = strsep(&nvp, "<")) != NULL) {
                 	//proto and active are mandatory
                        if (vstrsep(b, ">", &desc, &proto, &server, &username, &passwd, &active, &idx) < 2)
                                continue;
			if(idx)
			{	
				if(vid == atoi(idx))
...
int check_vpn_onoff(int onoff)
{
	int vpnc_idx=-1,vpnc_unit=-1;

	vpnc_idx=nvram_get_int("btnsw_vpn_idx");
	vpnc_unit=find_vpnc_unit_by_idx(vpnc_idx);
	if(vpnc_unit==-1)
	{
		//_dprintf("error, btnsw_vpn_idx invalid !\n");
		return -1;
	}

	if(onoff)
	{	
		//_dprintf("vpnc profile[%d] on\n",vpnc_unit);
		set_vpnc_active_by_unit(vpnc_unit,"1");
	}	
	else
	{
		//_dprintf("vpnc profile[%d] off\n",vpnc_unit);
		set_vpnc_active_by_unit(vpnc_unit,"0");
	}	
...
int check_sdn_onoff(int onoff)
{

	int sdn_idx=-1,err=0;
	sdn_idx=nvram_get_int("btnsw_sdn_idx");
	err=set_sdn_active_by_unit(sdn_idx,onoff);
	switch(err)
	{
		case 0:
			//_dprintf("update sdn_rl and restart wifi\n");
			sync_apgx_to_wlunit();
			restart_wireless();
			doSystem("rc rc_service restart_sdn");
			break;
		case -1:
			//_dprintf("get nvram sdn_rl fail\n");
			break;
		case -2:
			//_dprintf("btnsw_sdn_idx invalid\n");
			break;
		case -3:
			//_dprintf("no need to change sdn_rl\n");
			break;
		default:
			break;	
	}	
...
int check_wifi_onoff(int onoff)
{
	int sw = 1,sp =0,i;     // record get_radio status
        int MAX = 0;   
        sp=0;
	MAX = num_of_wl_if();
        for(i = 0; i < MAX; i++) {
	...
	switch(onoff)
	{
		case 0:
			if(sw || sp)
			{
				if(sp) //check if one radio is on
				{
        				for(i = 0; i < MAX; i++) 
					{
                				SKIP_ABSENT_BAND(i);
	...
	{		
		...
		switch(btn_mode) {
			...
			case 1:
				...
				break;
			case 2:
				//_dprintf("sw-btn: led %s\n",cmd?"on":"off");
				doSystem("rc rc_service \"start_ledctrl %d\"",cmd);
				break;
			case 3: 
				if((uptime()-tmp)>10 || sw_runtime ==-99)
				{	
					exec=1;
					if(sw_runtime != -99 && cmd == pre_cmd) //skip the same cmd
						break;
					//_dprintf("sw-btn: wifi %s\n",cmd?"on":"off");
					doSystem("rc rc_service \"start_wifictrl %d\"",cmd);
					tmp=uptime();
					pre_cmd=cmd;
				}
				...
				break;
			case 4:
				//_dprintf("sw-btn: vpn[%d] %s\n",nvram_get_int("btnsw_vpn_idx"),cmd?"on":"off");
				doSystem("rc rc_service \"start_vpnctrl %d\"",cmd);
				break;
			case 5:	
				if((uptime()-tmp)>10 || sw_runtime ==-99)
                                {
					...
					//_dprintf("sw-btn: sdn[%d] %s\n",nvram_get_int("btnsw_sdn_idx"),cmd?"on":"off");
                                        doSystem("rc rc_service \"start_sdnctrl %d\"",cmd);
                                        tmp=uptime();
                                        pre_cmd=cmd;
                                }
                                else  // execute after 10 sec
...
int btn_switch_main(void)
{
	int res;
	FILE *fp;
	unlink("/var/run/sw_btnd.pid");
        if ((fp = fopen("/var/run/sw_btnd.pid", "w")) != NULL)
        {
                fprintf(fp, "%d", getpid());
                fclose(fp);
        }
	...
	attrp = &attr;
        ...
        res=pthread_create(&thread_btn,&attr,(void *)&btn_detect,NULL);
	 if (res != 0) 
	 {
	 	_dprintf("sw-btn thread fail...\n");
		return 0;
	 }	 
...
int realip_main(int argc, char *argv[])
{
	char *stun_server_list[] = {
		...
		"stun.pjsip.org",	// SG
		// "stun.freeswitch.org",	// US
		// "stun.linphone.org",	// UK
		// "stun.nfon.net",	// DE
		// "stun.infra.net",	// DE
		// "stun.dus.net",	// DE
		// "stun.wifirst.net",	// FR
